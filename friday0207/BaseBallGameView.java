...
public class BaseBallGameView extends JFrame {
	...
	//********* 화면과 관련된 코드 추가 시작 **************
	//JFrame jf_bbgame = new JFrame();  => 상속해서 필요없음. // JFram = 창을띄워주는 명령어
	
	//JMenuBar는 JFrame 안에 메뉴바를 추가하기
	JMenuBar jmb_bbgame = new JMenuBar();
	
	//JMenu는 JMenuBar 안에 들어갈 메뉴 추가하기
	...
	//전역변수를 선언하여 문제를 해결할 수 있다.
	// - 생성자의 파라미터로 배열의 주소번지를 받게 되는데 이 값을 사용하는 곳이 
	//	  생성자 안에서가 아니라 initDisplay메소드 안에 setTitle메소드에서 사용해야 하기 때문에 파라미터로 넘어온 값을 반드시 전변과 초기화 해야 합니다.
	
	//파라미터자리는 변수를 선언하는 자리 입니다. 
	//초기화는 일어나지 않는다. - 생성하는 자리가 아니기 떄문.
	public BaseBallGameView(String result[]) { // #131(null) -> 60 : null값을 들고옴.
		this.result = result; // -> this를 붙이면 전변.
		if(this.result == null) {
	...
	public void initDisplay() {
		...
		/*
		 * 학습목표 : 이전 버전에서는 friday0131소스 이벤트처리를 직접 하였다. - ActionListener했다. 
		 */
		//디폴트 생성자는 JVM이 만들어줄 수 있어요  왜냐하면 파라미터가 없기 때문이죠
		//파라미터가 있는 생성자는 내가 대신 해줄 수 없다. - 왜냐하면 네 생각을 난 알 수가 없으니까... 
		BsaeBallGameEvent bbEvent =  new BsaeBallGameEvent(this); //========> 20200207 복습 이슈
		...
		jbtn_exit.addActionListener(bbEvent); //***이벤트 처리 순서 3번.
		jmi_oracle.addActionListener(bbEvent); 
		jmi_exit.addActionListener(bbEvent); //***이벤트 처리 순서 3번.
		...
		add("Center",jp_center); // jf_bbgame. => 상속되어 메소드를 호출할 때 주소번지를 빼서 사용 무방.
		add("East",jp_east); // jf_bbgame. => 상속되어 메소드를 호출할 때 주소번지를 빼서 사용 무방.
		...
		//************ 메뉴바 추가 끝 ***********
		setTitle("야구숫자게임 - " + result[0] + "[" + result[1] + "]"); // jf_bbgame. => 상속되어 메소드를 호출할 때 주소번지를 빼서 사용 무방.
		setSize(400,400); // jf_bbgame. => 상속되어 메소드를 호출할 때 주소번지를 빼서 사용 무방.
		setVisible(true); // jf_bbgame. => 상속되어 메소드를 호출할 때 주소번지를 빼서 사용 무방.
	}
	
	//************ 화면 처리 끝 *****************
	public static void main(String[] args) {
		new BaseBallGameView(null); //앞에 주소번지는 생성자 안에서 호출하기 때문에 쓸 필요가 없다.
	}
