...
public class LoginForm implements ActionListener {
	...
	public String[] login(String u_id, String u_pw) {
		...
		try {
			...
			sb.append(" 		 WHERE mem_id=?)	 "); // -> ? 앞에는 : 이 붙으면 안된다. , =: 는 오라클 언어.
			sb.append(" 		,-1) status	 		 ");		
			sb.append(" FROM dual	 				 ");		
			...
			if("1".equals(status)) { //아이디가 존재하니?
				...
				if(rs.next()) {
					...
					result[1] = db_id; //사용자가 입력한 id.
				}
				//아래에서 비밀번호가 맞지 않습니다. 메시지는 result배열에 담겨야 한다.
				//왜냐하면 리턴타입을 String에서 String[]로 바꾸었고 그 바뀐 정보를 if문에서 비교해야 하니까 더이상 
				//mem_name에 값은 if문에서 역활이 없어진 것이죠.
			...
			//에러를 해결하기 위해서는 최대한 많은 힌트를 끌어 모아야 한다.
			//에러메시지를 JVM이 statc영역에 관리하는데 그 이력과 라인번호를 같이 출력해줌.
			e.printStackTrace(); // => printStackTrace 기억하기.
	...
	public void initDisplay() {
		//로그인 버튼을 눌렀을 때 이벤트 감지가 되면 actionPerformed메소드를 호출하는데 이 메소드를 구현하는 클래스와 연결하기 
	...
	public void actionPerformed(ActionEvent e) {
		...
		//로그인 버튼을 누른거야?
		if(obj == jbtn_login) {
			String u_id = jtf_id.getText(); //텍스트 필드에 입력한 id
			String u_pw = jtf_pw.getText(); //텍스트 필드에 입력한 pw
