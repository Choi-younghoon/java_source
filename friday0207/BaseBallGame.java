...
/* 이벤트 처리 순서
 	...
 	3. jbtn_exit.addActionListener(this) 호출하여 이벤트 소스와 이벤트처리를 담당하는 클래스를 연결하기
 */
public class BaseBallGame implements ActionListener {
	//*********화면과 관련된 코드 추가 시작**************
	JFrame jf_bbgame = new JFrame(); // JFram = 창을띄워주는 명령어
	...
	public void initDisplay() {
		...
		jbtn_exit.addActionListener(this); //***이벤트 처리 순서 3번.
		jmi_exit.addActionListener(this); //***이벤트 처리 순서 3번.
	...
	@Override
	public void actionPerformed(ActionEvent e) { //*** 처리 순서중 2번째 
		//JVM이 감지한 이벤트 소스[jbtn_exit, jtf+input]로 부터 
		Object obj = e.getSource(); //여러개 받을 수 있기 때문에 Object 사용. / JButton을 사용하게 되면 한개만 받게되서 사용 x
		//너 그만 할거야??
		if(obj == jbtn_exit) {
			exitGame();
		}
		if(obj == jmi_exit) {
			//자바가상머신과 이 어플하고의 연결괴리를 끊김
			//System.exit(0);
			exitGame(); //-> 나가리 버튼 처리 메소드를 만들어서 이렇게 하면 하나로 관리가 가능하다.
		}
	}
