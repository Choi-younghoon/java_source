...
public class BsaeBallGameEvent implements ActionListener {
	DBConnectionMgr dbMgr = null;
	Connection 		con	  = null;
	BaseBallGameView bbView = null; //복습할때 7,8번 비교 : 왜 7번은 null인데 8번은 값을 주었나?					=======>20200207 복습 이슈
	//BaseBallGameLogic bbLogic = new BaseBallGameLogic(); // = View는 Event 왔을때 생성이 되어있는 상태(#54)   ======>20200207 복습 이슈
	//게임을 진행하는 동안에는 계속 그 숫자를 기억하고 있다가 1씩 증가되야 하니까...
	//전역변수로 해야 함.
	int cnt = 0; //회차를 출력할 변수 선언
	public BsaeBallGameEvent(BaseBallGameView bbView) {  // =====> 20200207복습이슈
		this.bbView = bbView;						
	}													
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == bbView.jbtn_dap) {
			...
			bbView.jbtn_dap.setEnabled(false); //버튼을 다시 못누르게 한다.
		}
