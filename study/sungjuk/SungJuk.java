...
public class SungJuk implements ActionListener {
	...
	//사용자가 입력한 인원수를 담을 변수
	//전역변수로 한 이유는 인원수를 듣기는 jtf_inwon에서 엔터쳤을 때 값이 결정된다.
	//그 때 결정된 3이 jbtn_account에서도 필요합니다.
	//왜냐하면 총점을 기준으로 석차를 구하기로 결정되었으므로 총점과 석차를 같이 관리할 2차배열을 선언하였기 때문입니다. 
	int inwon = 0;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == jbtn_account) {
			...
			//인원수는 어떻게 가져오지? - 전역변수로 선언하고 사용하는게 좋을거같다.
			//왜냐하면 다른 이벤트에서도 필요하기 때문이다.
			...
			for(int i=0; i<inwon; i++) {
				...
				//구한 총점을 DfaultTableModel 객체에 담기
				...
				imsi[i][0] = tot;
				imsi[i][1] = 1; //조건을 수령하지 않을 경우가 발생할 수 있다. 이때, 0등이 안되니까 초기화를 1로 변경하였다.
			}//end of for
				//석체 매기기
				for(int i=0; i<inwon; i++) {
