...
//이 메소드 안에서는 무엇을 하지? - 기다리는것[Thread.sleep(1000)], 듣기[ois.readObject()]와 말하기[oos.writeObject("메시지")].
public class ServerBank extends JFrame implements Runnable {
	
	//////////////////////전역변수 선언하기 시작////////////////////
	Socket socket = null; //인스턴스화 X - 복사본이 아니라 원본을 사용해야 하니까 반드시 null로 초기화.
	int port = 3000;
	//서버소켓은 사용자가 접속해 오기를 기다립니다. 언제까지 기다려야 할지 알 수 없다.
	...
	//List는 인터페이스이므로 단독으로 인스턴스화 불가하니까 
	//구현체 클래스 중에서 여러 사람을 손실없이 관리할 수 있는 Vector객체를 생성할것.
	//클라이언트가 접속을 했을 때 스레드를 가동시킴.
	//ServerBankThread가 서버측에서 생성한 클래스이지만 그 안에 담긴 정보는 클라이언트에 대한 정보를 담고있다.
	//클라이언트가 접속 성공하면 일반소켓에게 서버소켓이 수집한 정보를 넘김.
	//이 정보를 넘겨 받으면 그 안에 클라이언트 정보가 담김.
	//스레드가 생성 되었을 때 그 때 Vector안에 add처리 할것. - 그래야 그 사람 정보를 유지 가능.
	...
	public static void main(String[] args) {
		...
		sb.initDisplay();
		//sb.start(); -> 호출불가 - Thread상속 받지 않았으니까 - 나는 Thread가 아님.
		//어떻게 해결하지 ? - 일단 Thread를 인스터스화 하고 생성자에 구현체클래스를 넣어줌
	...
	public void run() {
		...
		try {
			server = new ServerSocket(port); //가게문 열고 기다리는중... 손님이 언제 올까(ip,port)
		} catch (Exception e) {
			e.printStackTrace();
		}//end of try
		...
		while(true) { // => 무한루프 - while탈출 불가.
			try {
			//클라이언트 측에서 접속해온 정보를 client소켓에게 넘김.
				socket = server.accept(); //대기
				jta_log.append("New Client connected" + socket.toString() + "\n");
				sbt = new ServerBankThread(this); //this == ServerBank 자신 - 원본 - 생성자호출
