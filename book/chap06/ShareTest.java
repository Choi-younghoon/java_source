 ...
 * 결론(완결편)
 	...
 	static이 붙은 변수는 인스턴스화 없이 사용할수 있다.
 	그리고 또 static이 붙은 변수는하나가 공유되는 것이다.
 	그렇기 때문에 위에서 methodA에서 출력한 j값과 main메소드 안에서 출력된 j값이 서로 같다.
 	그러나 변수 i는 static이 없으니까... 하나가 아니라 복사본이 변경되는 것이다.
 	따라서 methodA에서 출력한 i값과 main메소드 안에서 출력된 i값이 서로 다를 수 밖에 없다.
 */
