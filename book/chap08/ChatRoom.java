...
public class ChatRoom extends JFrame {
	...
	//JTalbe에 들어갈 데이터 영역을 생성하기.
	//데이터는 존재하지 않으므로 0을 주었고 컬럼의 수는 톡방명, 총인원수, 현재 참여자 명단 이렇게 3명이므로 3을 주었음.
	String data[][] = new String[0][3];
	
	//실제로 데이터를 담을 수 있는 자바에서 제공되는 클래스이다.
	//화면에 붙일때 헤더정보들은 2차배열에 포함시키지 않음.
	DefaultTableModel dtm_room = new DefaultTableModel(data, cols);
	
	//JTable은 양식을 제공할 뿐 데이터는 DefaultTableModel에 초기화되어야 한다.
